...
app.post('/analyze-report', async (req, res) => {
  ...
  console.log(`Analyzing new report manually triggered: ${reportId}`);

  try {
    const docRef = admin.firestore().collection('reports').doc(reportId);
    ...
    if (!report.photoURL && !report.description) {
      console.log('No photo or description to analyze');
      return res.json({ success: true, message: 'No content to analyze' });
    }
    ...
    console.error(`Error in analyze-report endpoint for ${reportId}:`, error);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
// Standalone Server Startup (for Render/Vercel)
if (require.main === module) {
  const PORT = process.env.PORT || 8080;
  app.listen(PORT, () => {
    console.log(`🚀 Standalone Backend API listening on port ${PORT}`);
  });
}
